...
Following are some key features of the code completion support provided by the plugin. 
...
Service templates corresponding to each available listener is provided in the list of completion items.
...
The `Fill Record required fileds` completion item is provided for filling the remaining fields of a `Record` typed value.
